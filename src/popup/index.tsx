...
function Popup() {
  ...
  // Check for pending reminder from Mattermost "Remind Me" action
  useEffect(() => {
    chrome.storage.local.get(STORAGE_KEYS.PENDING_REMINDER, (result) => {
  ...
  return (
    <div className={`w-[400px] h-[600px] flex flex-col bg-slate-50 relative ${(isCreating || editingReminder) ? '' : 'pb-[68px]'}`}>
      <div className="bg-white px-5 pt-4 pb-0 flex flex-col gap-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] relative z-10">
        ...
        <div className="flex gap-6">
          {tabs.map((tab) => (
            <button
              key={tab}
              onClick={() => { setActiveTab(tab); setShowSettings(false); setIsCreating(false); setEditingReminder(null); }}
      ...
      <div className={`flex-1 overflow-y-auto ${(isCreating || editingReminder) ? '' : 'p-4'}`}>
        {loading ? (
          ...
          <ReminderForm 
            ...
            onCancel={() => { setIsCreating(false); setEditingReminder(null); }} 
          />
        ) : showSettings ? (
          <SettingsPanel />
        ) : (
