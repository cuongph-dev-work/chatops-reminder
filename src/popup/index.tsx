...
function Popup() {
  ...
  return (
    <div className="w-[400px] h-[600px] flex flex-col bg-slate-50 relative pb-16">
      <div className="bg-white px-5 pt-4 pb-0 flex flex-col gap-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] relative z-10">
        ...
        <div className="flex gap-6">
          {tabs.map((tab) => (
            <button
              key={tab}
              onClick={() => { setActiveTab(tab); setShowSettings(false); setIsCreating(false); }}
      ...
      <div className={`flex-1 overflow-y-auto ${isCreating ? '' : 'p-4'}`}>
        {loading ? (
          <p className="text-sm text-slate-400 text-center py-8">Loading...</p>
        ) : isCreating ? (
          <ReminderForm onCancel={() => setIsCreating(false)} />
        ) : showSettings ? (
          <SettingsPanel />
        ) : (
          <>
